...
public class Tester {
	...
	public static void stringSorting(){
		...
		Sort<MyString> qs6 = new ParallelQuickSort<MyString>();
		Sort<MyString> qms = new QuickMergeSort_2<MyString>();//修正
		...
		if(!MyArrayUtil.checker(stringArray)){
...
//		long stop9 = System.currentTimeMillis();
		...
		if(!MyArrayUtil.checker(array13)){

			System.out.println("OUT");
			MyArrayUtil.print(array);
			MyArrayUtil.print(array13);
...
//				System.out.println("qms time : "+(stop13 - start13));
//				System.out.println("qms2 time : "+(stop14 - start14));
//				System.out.println("qms3 time : "+(stop9 - start9));
	...
	private static void doubleSorting() {
		...
		MyDouble[] doubleArray3 = Arrays.copyOf(doubleArray,doubleArray.length);
		MyDouble[] doubleArray4 = Arrays.copyOf(doubleArray,doubleArray.length);
		MyDouble[] doubleArray5 = Arrays.copyOf(doubleArray,doubleArray.length);
		...
		Sort<MyDouble> qs6 = new ParallelQuickSort<MyDouble>();
		Sort<MyDouble> ms = new MergeSort<MyDouble>();
		Sort<MyDouble> qms_0 = new QuickMergeSort<MyDouble>();//修正
		Sort<MyDouble> qms = new QuickMergeSort_2<MyDouble>();//修正
		...
		long qms_0Start = System.currentTimeMillis();
		qms_0.sort(doubleArray5);
		long qms_0Stop = System.currentTimeMillis();

		long Start = System.currentTimeMillis();
		Arrays.sort(doubleArray6);
		long Stop = System.currentTimeMillis();

		//MyArrayUtil.print(doubleArray10);
		Sort<MyDouble> qs5 = new QuickSort5<MyDouble>();
		long start9 = System.currentTimeMillis();
		qs5.sort(doubleArray7);
		long stop9 = System.currentTimeMillis();
		//MyArrayUtil.print(doubleArray10);

		long start10 = System.currentTimeMillis();
		qms.sort(doubleArray8);
		long stop10 = System.currentTimeMillis();
		...
		System.out.println("quickMergeSort_0 time  :"+(qms_0Stop - qms_0Start)); //修正
		System.out.println("Arrays Sort time         :"+(Stop - Start)); //修正
		System.out.println("forkJoin quickSort time  :"+(stop9 - start9)); //修正
		System.out.println("quickMergeSort_2 time      :"+(stop10 - start10)); //修正
	}


	public static void pinPointCheak(){
		MyInteger[] array = new MyInteger[19];
		int[] data = {5 ,17, 14, 14, 16, 19, 10, 4, 7, 8, 16, 5, 17, 12, 16, 17, 15, 15, 9};

		for(int i = 0;i < array.length;i++)
			array[i] = new MyInteger(data[i]);

		MyInteger[] afterArray = Arrays.copyOf(array,array.length);

		Sort<MyInteger> ns = new NewSort<MyInteger>();

		ns.sort(afterArray);

		if(!MyArrayUtil.checker(afterArray))
			System.out.println("OUT");
		else
			System.out.println("SAFE");

		MyArrayUtil.print(array);
		MyArrayUtil.print(afterArray);
	...
	public static void intSorting(){
		...
		System.out.println("-------QuickMergeSort3------");
		long start12 = System.currentTimeMillis();
		qms3.sort(array12);
		long stop12 = System.currentTimeMillis();


		System.out.println("-------QuickMergeSort4------");
		long start13 = System.currentTimeMillis();
		qms4.sort(array13);
		long stop13 = System.currentTimeMillis();
		...
		System.out.println("ParallelQuickSort time   :"+(stop6 - start6)); //修正
		System.out.println("ArraysSort time          :"+(stop4 - start4)); //修正
		System.out.println("Merge Sort time          :"+(stop7 - start7)); //修正
		System.out.println("New Sort time            :"+(stop8 - start8)); //修正
		System.out.println("forkJoin quickSort time  :"+(stop9 - start9)); //修正
		System.out.println("Quick Merge Sort time    :"+(stop10 - start10)); //修正
		System.out.println("Prallel Merge Sort time  :"+(stop11 - start11)); //修正
		System.out.println("Prallel Merge Sort3 time :"+(stop12 - start12)); //修正
		System.out.println("Prallel Merge Sort4 time :"+(stop13 - start13)); //修正
